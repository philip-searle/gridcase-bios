...
FD_BDA		PROGRAM OutFile=build/fd_bda.obj
		...
		EXTERN	FdGetOpStartState, FdSetOpStartState
		EXTERN	FdResetSelect, FdDiskChange

		PUBLIC	FdGetDriveInfo
		PUBLIC	FdResetMediaState
		PUBLIC	FdSetDriveInfo
		PUBLIC	FdSyncDtr
		PUBLIC	FdTryNextMediaType
		PUBLIC	FdUpdateDrvInfo
		PUBLIC	sub_FACA1
...
FdTryNextMediaType	PROC
		...
		; need to do anything
		test	[FdDriveInfo], FDDI_DRIVES_OK, DATA=BYTE
		jz	.leaveFunction
...
.chosenMedia	; Compare the new media type in AH to the operational
		...
		jnz	.haveErrorCode
		mov	[FdScratch.returnAh], INT13STAT_BADADDRMARK, DATA=BYTE
...
.leaveSuccess	and	[FdScratch.returnAh], 0, DATA=BYTE
		call	FdSetMediaState2
		retn
		ENDPROC	sub_FACA1
